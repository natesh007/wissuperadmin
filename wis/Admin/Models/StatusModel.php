...
class StatusModel extends Model {
    ...
    protected $allowedFields = ['StatusName', 'Status', 'CreatedBy', 'CreatedDate', 'UpdatedBy', 'UpdatedDate'];
    ...
    function get_status($page, $perpage, $keyword, $status) 
    {
        $start_from = ($page - 1) * $perpage;
        $query = 'SELECT om.*, a.Name FROM status om left join admins a on a.AID = om.UpdatedBy';
        if ($keyword !=''&& $status !='') {
            $query .=' where om.StatusName  like "%'. $keyword . '%" AND om.Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $query .=' where om.StatusName  like "%'. $keyword . '%"';
        }
        ...
        $data['results'] = $this->db->query($query)->getResultArray();
        $countquery = 'SELECT count(StatusID) as ttl_rows FROM status';
        if ($keyword !=''&& $status !='') {
            $countquery .=' where StatusName  like "%'. $keyword . '%" AND Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $countquery .=' where StatusName like "%'. $keyword . '%"';
        }
