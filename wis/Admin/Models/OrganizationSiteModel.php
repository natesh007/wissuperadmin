...
class OrganizationSiteModel extends Model {
    protected $table='organizationsite';
    protected $primaryKey='OrgSiteID';
    protected $allowedFields = ['Site', 'Status', 'CreatedBy', 'CreatedDate', 'UpdatedBy', 'UpdatedDate'];
    ...
    function get_organizationsite($page, $perpage, $keyword, $status) 
    {
        $start_from = ($page - 1) * $perpage;
        $query = 'SELECT om.*, a.Name FROM organizationsite om left join admins a on a.AID = om.UpdatedBy';
        if ($keyword !=''&& $status !='') {
            $query .=' where om.Site  like "%'. $keyword . '%" AND om.Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $query .=' where om.Site  like "%'. $keyword . '%"';
        }
        ...
        $organizationsite['results'] = $this->db->query($query)->getResultArray();
        $countquery = 'SELECT count(OrgSiteID) as ttl_rows FROM organizationsite';
        if ($keyword !=''&& $status !='') {
            $countquery .=' where Site  like "%'. $keyword . '%" AND Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $countquery .=' where Site like "%'. $keyword . '%"';
        }
        ...
        $organizationsite['ttl_rows'] = $row->ttl_rows;
        $adjacents = "2";
        ...
        $organizationsite['pagelinks'] = $pagelinks;
