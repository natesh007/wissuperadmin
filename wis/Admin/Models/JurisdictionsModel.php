...
class JurisdictionsModel extends Model {
    protected $table='jurisdictions';
    protected $primaryKey='JurisdictionsID';
    protected $allowedFields = ['Jurisdictions', 'Status', 'CreatedBy', 'CreatedDate', 'UpdatedBy', 'UpdatedDate'];
    ...
    function get_jurisdictions($page, $perpage, $keyword, $status) 
    {
        $start_from = ($page - 1) * $perpage;
        $query = 'SELECT om.*, a.Name FROM jurisdictions om left join admins a on a.AID = om.UpdatedBy';
        if ($keyword !=''&& $status !='') {
            $query .=' where om.Jurisdictions  like "%'. $keyword . '%" AND om.Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $query .=' where om.Jurisdictions  like "%'. $keyword . '%"';
        }
        ...
        $jurisdictions['results'] = $this->db->query($query)->getResultArray();
        $countquery = 'SELECT count(JurisdictionsID) as ttl_rows FROM jurisdictions';
        if ($keyword !=''&& $status !='') {
            $countquery .=' where Jurisdictions  like "%'. $keyword . '%" AND Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $countquery .=' where Jurisdictions like "%'. $keyword . '%"';
        }
        ...
        $jurisdictions['ttl_rows'] = $row->ttl_rows;
        $adjacents = "2";
        ...
        $jurisdictions['pagelinks'] = $pagelinks;
