 ...
 *   Michael Ansaldi
 *   Ryan Tse
 *	 Srishu Indrakanti/Ranga Kovvuri
 */
class MySQL {
	// SET THESE VALUES TO MATCH YOUR DATA CONNECTION
	// private $db_host    = "13.126.111.226"; // server name
	// private $db_user    = "dovespoc";          // user name
	// private $db_pass    = "Dovespoc@123";          // password
	// private $db_dbname  = "dovespocdb"; // version 2.0
	private $db_host    = DBHOST; // server name
	private $db_user    = DBUSER;          // user name
	private $db_pass    = DBPASS;          // password -c%%#?nk]Lag
	private $db_dbname  = DBNAME; // version 2.0
	 ...
	 */
	public $canThrowExceptions = false;
	 ...
	 */
	public function tryUpdateOrInsert($tableName, $valuesArray, $whereArray) {
		$this->resetError();
		$this->getRows($tableName, $whereArray);
	...
	/**
	 * closeDB current MySQL connection
	 *
	...
	public function isLastRow() {
		$this->resetError();
		if ($this->isConnected()) {
			if ($this->active_row >= ($this->getRowsCount())) {
				return true;
			} else {
				return false;
			}
	 ...
	 */
	public function getColumnsCount($table = "") {
		$this->resetError();
		if (empty($table)) {
		...
		if (empty($table)) {
			if ($this->getRowsCount() > 0) {
				if (is_numeric($column)) {
					...
					$field = mysqli_fetch_field_direct($this->last_result, $this->getColumnIndex($column));
					return $field->type;
				}
			} else {
				return false;
	 ...
	 */
	public function getColumnIndex($column, $table = "") {
		$this->resetError();
		$columnNames = $this->getColumnNames($table);
		if (! $columnNames) {
			return false;
	...
	public function getColumnLength($column, $table = "") {
		...
		if (empty($table)) {
			if (is_numeric($column)) {
				...
				$columnID = $this->getColumnIndex($column);
			}
			if (! $columnID) {
				return false;
			} else {
   ...
   /**
	...
	public function getHTML($showCount = true, $styleTable = null, $styleHeader = null, $styleData = null) {
		...
		if ($this->last_result) {
			if ($this->getRowsCount() > 0) {
				$html = "";
				if ($showCount) $html = "Record Count: " . $this->getRowsCount() . "<br />\n";
	...
	*/
	...
	public function getXML() {
		...
		if (is_resource($this->last_result)) {
			...
			$root->setAttribute('rows',
				($this->getRowsCount() ? $this->getRowsCount() : 0));
			...
			$root->setAttribute('query', $this->last_sql);
			if ($this->getErrorString()) {
				$root->setAttribute('error', $this->getErrorString());
			} else {
	...
	public function hasRecords($sql = "") {
		if (strlen($sql) > 0) {
			...
			if ($this->getErrorString()) return false;
		}
		if ($this->getRowsCount() > 0) {
			return true;
		} else {
	...
	public function moveToEnd() {
		$this->resetError();
		$this->active_row = $this->getRowsCount() - 1;
		if (! $this->getErrorString()) {
			if (! $this->seekToRow($this->active_row)) {
				return false;
			} else {
				return true;
			}
	 ...
	 * @return boolean Returns TRUE on success or FALSE on error
	 */
	public function openDB($database = null, $server = null, $username = null,
	 ...
	 */
	public function executeQueryAndGetArray($sql, $resultType = MYSQLI_BOTH) {
		$this->executeQuery($sql);
		if (! $this->getErrorString()) {
			return $this->getRecordsFromLastQueryAsArray($resultType);
		} else {
			return false;
		}
	}
	 ...
	 */
	public function executeQueryAndGetFirstRow($sql) {
		$this->executeQuery($sql);
		if ($this->getRowsCount() > 0) {
	 ...
	 */
	public function getFirstRowAsArray($sql, $resultType = MYSQLI_BOTH) {
		$this->executeQuery($sql);
		if ($this->getRowsCount() > 0) {
			return $this->getCurrentRowAsArray(null, $resultType);
		} else {
			return false;
		}
	}
	 ...
	 */
	public function getFirstRowFirstColumn($sql) {
		$this->executeQuery($sql);
		if ($this->getRowsCount() > 0 && $this->getColumnsCount() > 0) {
			$row = $this->getCurrentRowAsArray(null, MYSQLI_NUM);
			return $row[0];
		} else {
			return false;
		}
	 ...
	 *                TRUE or FALSE for all others i.e. UPDATE, DELETE, DROP
	 */
	public function saveQueryExecutionTimeMicroSeconds($sql) {
		$this->startTimer();
	 ...
	 */
	public function getRecordsFromLastQuery() {
		return $this->last_result;
	}
	 ...
	 */
	public function getRecordsFromLastQueryAsArray($resultType = MYSQLI_BOTH) {
		$this->resetError();
		if ($this->last_result) {
	 ...
	 */
	public function getCurrentRow($optional_row_number = null) {
		$this->resetError();
		if (! $this->last_result) {
			$this->setError("No query results exist", -1);
			return false;
	 ...
	 */
	public function getCurrentRowAsArray($optional_row_number = null, $resultType = MYSQLI_BOTH) {
		$this->resetError();
		if (! $this->last_result) {
			$this->setError("No query results exist", -1);
			return false;
	 ...
	 */
	public function seekToRow($row_number) {
		$this->resetError();
	 ...
	 */
	public function getRows($tableName, $whereArray = null, $columns = null,
	...
	/**
	 * Returns last measured duration (time between startTimer and stopTimer)
	 *
	 * @param integer $decimals (Optional) The number of decimal places to show
	 * @return Float Microseconds elapsed
	 */
	public function getTimerDuration($decimals = 4) {
	...
	public function isValueUnique($tableName,$fieldName,$value) {
		$db = new MySQL();
		if(!empty($tableName) and !empty($fieldName) and !empty($value)){
			$sql = "SELECT * FROM `".$tableName."` WHERE `".$fieldName."`='".mysqli_real_escape_string($db->mysql_link,$value)."'";
