...
class OrganizationTypeModel extends Model {
    ...
    function get_organization_types($page, $perpage, $keyword, $status) 
    {
        $start_from = ($page - 1) * $perpage;
        $query = 'SELECT ot.*, a.Name FROM organization_type ot left join admins a on a.AID = ot.UpdatedBy';
        if ($keyword !=''&& $status !='') {
            $query .=' where ot.OrganizationType  like "%'. $keyword . '%" AND ot.Status = '.$status;
        }

        else if ($keyword !=''&& $status=='') {
            $query .=' where ot.OrganizationType  like "%'. $keyword . '%"';
        }
        ...
        if ($keyword !=''&& $status !='') {
            ...
            $countquery .=' where OrganizationType like "%'. $keyword . '%"';
        }
        ...
        $organization_type['ttl_rows'] = $row->ttl_rows;
        $adjacents = "2";
        ...
        $organization_type['pagelinks'] = $pagelinks;
